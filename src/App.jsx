...
function InvoicePrint({ res, client, room, settings, cur }) {
  ...
  const handle = () => {
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #c9a84c;padding-bottom:20px;margin-bottom:30px">
      <div>${settings.logoUrl?`<img src="${settings.logoUrl}" style="height:80px;object-fit:contain;margin-bottom:8px;display:block"/>`:`<div style="font-size:36px">🏨</div>`}<h1>FACTURE</h1><div style="font-size:18px;color:#555">${settings.hotelName}</div></div>
...
<Inp label="🏙️ Ville (pour météo)" value={f.city||""} onChange={set("city")} placeholder="Casablanca"/>
        ...
        <div style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:12,padding:22}}>
          <div style={{color:GOLD,fontFamily:"Georgia,serif",fontSize:15,marginBottom:16}}>🏨 {t.myHotels||"My Hotels"}</div>
          <div style={{color:"#64748b",fontSize:13,marginBottom:16}}>{t.comingSoon||"Coming soon"}</div>
          <div style={{background:"rgba(201,168,76,0.08)",border:"1px solid rgba(201,168,76,0.2)",borderRadius:8,padding:"10px 14px",fontSize:12,color:GOLD}}>
            🔜 {t.enterprisePlan||"Available in Enterprise plan"} — <a href="/pricing" style={{color:GOLD,fontWeight:700}}>{t.upgrade||"Upgrade"}</a>
          </div>
        </div>
        <div style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:12,padding:22}}>
